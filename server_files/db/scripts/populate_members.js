...
fetch('https://api.propublica.org/congress/v1/116/house/members.json', { method: 'GET', headers: headers })
    .then(data => {
      return data.json()
    })
    .then(json => {
      for (let i = 0; i < json.results[0].members.length; i++) {

        if (ids_saved[json.results[0].members[i].id] == true) {
          continue;
        }
        ids_saved[json.results[0].members[i].id] = true

        // Formatting for sql
        if (i != json.results[0].members.length - 1) {
          ids_to_save += `("${json.results[0].members[i].id}"),`
        } else {
          ids_to_save += `("${json.results[0].members[i].id}")`
        }
      }
      saveMembers(ids_to_save)
    });

// const json = await houseData.json();